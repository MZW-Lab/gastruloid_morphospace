...
%Plotting Function
    if flag == 1
        ...
        imsave(inv_dapi);
        %imwrite(inv_dapi, 'Dapi_sample.tiff');
    end

%% Background Removal
    [colonymask, colonymask2, colonymask3, colonymask4, colonymask5, cm4_CC] = background_rm(nuc_diam, im_DAPI, erode_diam);
%Plotting Function
    if flag == 1
        figure(1)
...
%Plotting Function
    if flag == 1
        figure(1)
        %dapi = imshow(BW);
        dapi = imshowpair(-im_DAPI, BW, 'diff');
        imsave(dapi);  
    end
